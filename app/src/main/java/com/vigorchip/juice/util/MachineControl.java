...
public class MachineControl {
    ...
    public MachineControl(List<ActionBean> allActionBeans, long curTimes, ControlListener listener) {
    ...
    public void startPlay() {
        ...
        int[] actionMethon = getStartActionMethon(curTimes);
     ...
     * @return
     */
    private int[] getStartActionMethon(long curtime) {
        ...
        for (int i = 0; i < allActionBeans.size(); i++) {
            ...
            if (curtime > stime_int && curtime <= etime_int) {
                if (!isSpeedControl) {//外部控制，所以不取此速度
                    ...
                    int endSpeed = Integer.valueOf(actionBean.getEndspeed());
                    int speed = startSpeed + (int) ((endSpeed - startSpeed) * (curtime - stime_int) * 1.0 / (etime_int - stime_int));
                    if (startSpeed == 1 && endSpeed == 5) {
                        if (curtime >= 1 && curtime < 1000) {
                            Logutil.e("人为设置speed=1");
                            speed = 1;
                        }
                        if (curtime >= 1000 && curtime < 2000) {
                            Logutil.e("人为设置speed=2");
                            speed = 2;
                        }
                        if (curtime >= 2000 && curtime < 3000) {
                            Logutil.e("人为设置speed=3");
                            speed = 3;
                        }
                        if (curtime >= 3000 && curtime < 4000) {
                            Logutil.e("人为设置speed=4");
                            speed = 4;
                        }
                        if (curtime >= 4000 && curtime < 5000) {
                            Logutil.e("人为设置speed=5");
                            speed = 5;
                        }
                    }
                    if (speed != curSpeed) {
