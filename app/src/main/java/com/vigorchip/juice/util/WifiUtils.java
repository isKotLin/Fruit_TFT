...
public class WifiUtils {
    ...
    public boolean removeWifiConfig(String SSID) {
        for (int i = 0; i < wifiConfigList.size(); i++) {
            if (wifiConfigList.get(i).SSID.equals(SSID)) {//地址相同
                WifiConfiguration wifiConfiguration = wifiConfigList.get(i);
                String pswd = wifiConfiguration.preSharedKey;
                Logutil.e("删除错误密码");
                return localWifiManager.removeNetwork(wifiConfiguration.networkId);
            }
        }
        return false;
    }
    ...
    public int AddWifiConfig(List<ScanResult> wifiList, String ssid, String pwd) {
        ...
        for (int i = 0; i < wifiList.size(); i++) {
            ...
            Logutil.e("AddWifiConfig",wifi.SSID+ "======"+ssid);
