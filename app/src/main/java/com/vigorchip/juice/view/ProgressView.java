...
//        Logutil.e("大圆半径=" + (radius + roundWidth / 2));
//        Logutil.e("小圆半径=" + (radius - roundWidth / 2));
...
//        canvas.drawCircle(centre, centre, radius - roundWidth / 2, paint); //画出圆环
        ...
        paint.setStrokeWidth(2); //设置圆环的宽度
        paint.setAntiAlias(true);  //消除锯齿
        canvas.drawCircle(centre, centre, radius + roundWidth / 2 - 1, paint); //画出圆环
        canvas.drawCircle(centre, centre, radius - roundWidth / 2, paint); //画出圆环
    ...
    public synchronized void setProgress(int progress) {
        ...
        if (progress <= max) {
            ...
            mHandler.removeMessages(SUCCESS_CODE);
            CurCount = 0;
            mHandler.sendEmptyMessage(SUCCESS_CODE);
        }
    }
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == SUCCESS_CODE) {
                if (CurCount < 10) {
                    CurCount++;
                    mHandler.sendEmptyMessageDelayed(SUCCESS_CODE, 10);
                    postInvalidate();
                }
            }
        }
