...
public class WheelView extends View {
    ...
    private int showSize = 3;             //展示Item的个数
    private float textSize = 24;          //文字的大小
    private boolean isCircle = false;     //是否为环形
    private int rate = 180;               //惯性滑动比率，rate越大，速率越快
    private int textColor = 0xFFFED703;   //文字颜色
...
//        canvas.drawRect(0, 0, width, height, coverPaint);
     ...
     * @return
     */
    public HashMap<Integer, Object> getCenterItem() {
        HashMap<Integer, Object> result = new HashMap<>();
        if (cacheNowItem >= 0) {
            result.put(1, cacheNowItem);
            result.put(2, data.get(cacheNowItem).backData);
            return result;
        } else {
            ...
            if (dataSize > 0) {
                result.put(1, nowChecked);
                result.put(2, data.get(nowChecked).backData);
            }
            return result;
        }
    }
...
//                lastY = y;
                ...
                if (!isCircle && scrollY < -(getRealHeight() - (showSize + 1) / 2 * itemHeight)) {
                    Logutil.e("上拉超出");
                    setCenterItem(getDataSize(false)-1);
                } else if (!isCircle && scrollY > (showSize - 1) / 2 * itemHeight) {
                    Logutil.e("下拉超出");
                    setCenterItem(0);
                } else {
                    float y = event.getRawY();
...
//            mScroller.springBack(0,(int)scrollY,0,0,minScrollY,maxScrollY);
        ...
        if (mSelectListener != null) {
            HashMap<Integer, Object> centerItem = getCenterItem();
            mSelectListener.onSelect((int) centerItem.get(1), (String) centerItem.get(2));
        }
    }
